...
class VoucherController extends Controller
{
    ...
    public function receiveVoucher(Request $request, $voucherId)
    {
        ...
        $user = Auth::user(); // Giả sử bạn đã sử dụng xác thực người dùng bằng Auth
        $existingVoucher = UserVoucher::where('user_id', $user->id)
            ->where('voucher_id', $voucherId)
            ->first();
        if ($existingVoucher) {
            return response()->json(['error' => 'Bạn đã nhận voucher này rồi'], 400);
        }
        $currentDate = now();

        if ($voucher->start_date > $currentDate || $voucher->end_date < $currentDate) {
            return response()->json(['error' => 'Voucher đã hết hạn hoặc chưa đến thời gian sử dụng'], 400);
        }

        $userVoucher = new UserVoucher();
        $userVoucher->user_id = $user->id;
        $userVoucher->voucher_id = $voucherId;
        $userVoucher->received_at = now();

        return response()->json(['success' => 'Voucher đã được nhận thành công']);
    }
