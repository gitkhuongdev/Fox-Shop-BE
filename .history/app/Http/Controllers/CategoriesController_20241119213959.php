...
public function api_paginate_products_by_category($slug, Request $request)
{
    ...
    // Lấy id của category dựa vào slug
    $category = Categories::where('slug', $slug)->first();

    // Kiểm tra nếu không tìm thấy category với slug
    if (!$category) {
        return response()->json(['message' => 'Category not found'], 404);
    }

    // Truy vấn danh sách sản phẩm dựa trên id của category
    $products = Products::join('product_categories', 'products.id', '=', 'product_categories.id_product')
        ->join('gallery', 'products.id', '=', 'gallery.id_parent')
        ->where('product_categories.id_categories', $category->id) // Sử dụng id từ category tìm được
        ->where('products.status', 1)
