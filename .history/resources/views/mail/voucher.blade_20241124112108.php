...
<body style="font-family: Arial, sans-serif; background-color: #f4f7fa;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; background-color: #fff; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); border-radius: 8px; text-align: center;">
        ...
        @if({{ $voucher->discount_type === "percentage" }})
            Giảm giá {{ $voucher->discount_value }}% cho đơn hàng từ {{ number_format($voucher->minimum_monney) }} trở lên</p>
        @elseif($order->id_payment == 2)
        Giảm giá {{ number_format($voucher->discount_value) }} VND cho đơn hàng từ {{ number_format($voucher->minimum_monney) }} trở lên</p>
        @endif
