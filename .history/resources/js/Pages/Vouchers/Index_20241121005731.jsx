...
function Index({ voucher }) {
    ...
    useEffect(() => {
        ...
        voucher.map((item) => {
            setStart(formatDate(item.start_date));
            setEnd(formatDate(item.end_date));
            // console.log("Voucher:", item);
            if (date > item.start_date) {
                // axios.post(`/admin/vouchers/status/${item.id}`).then((res) => {
                ...
                //     }
                // });
                console.log("ko hop le", date);
            } else console.log("hop le");
        });
    }, [voucher]);
    const columns = [
        {
