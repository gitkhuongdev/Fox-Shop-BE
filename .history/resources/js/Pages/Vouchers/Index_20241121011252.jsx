...
function Index({ voucher }) {
    ...
    useEffect(() => {
        ...
        voucher.map((item) => {
            ...
            if ((date < startDate || date > endDate) && item.status === "active") {
                axios.post(`/admin/vouchers/status/${item.id}`, {status : item.status}).then((res) => {
            ...
            } else if ((date >= startDate || date <= endDate) && item.status === "inactive") {
                console.log("ko hợp lệê",  startDate,date, endDate);
            } else {
