...
function Index({ voucher }) {
    ...
    useEffect(() => {
        if (!loading && voucher.length > 0) {
          const date = getCurrentDate(); // Giả sử bạn đã có hàm getCurrentDate()
          console.log("Ngày hiện tại:", date);
    
          voucher.forEach((item) => {
            const startDate = formatDate(item.start_date); // Giả sử bạn có hàm formatDate
            const endDate = formatDate(item.end_date);
            console.log("Ngày bắt đầu:", startDate);
            console.log("Ngày kết thúc:", endDate);
    
            // Kiểm tra nếu ngày hiện tại trong khoảng start và end
            if (date >= startDate && date <= endDate) {
              if (item.status === "inactive") {
                console.log("Voucher đang inactive, chuyển thành active");
                axios
                  .post(`/admin/vouchers/status/${item.id}`, { status: "active" })
                  .then((res) => {
                    if (res.data.check) {
              ...
              if (item.status === "active") {
                ...
                axios
                  .post(`/admin/vouchers/status/${item.id}`, { status: "inactive" })
                  .then((res) => {
                    if (res.data.check) {
            ...
            // Kiểm tra nếu voucher đã hết hạn sử dụng
            if (item.usage_limit <= 0 && item.status === "active") {
              console.log("Voucher hết lượt sử dụng, chuyển thành inactive");
              axios
                .post(`/admin/vouchers/status/${item.id}`, { status: "inactive" })
                .then((res) => {
                  if (res.data.check) {
    ...
    const handleReceiveVoucher = (voucherId) => {
        axios
            .post(`/admin/receive-voucher/${voucherId}`, { userid: userId })
            .then((response) => {
                console.log("ok");
            })
            .catch((error) => {
