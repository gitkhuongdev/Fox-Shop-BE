...
function Index({ voucher }) {
    ...
    useEffect(() => {
        const date = getCurrentDate();  // Lấy ngày hiện tại dưới dạng YYYY-MM-DD
    
        voucher.map((item) => {
            const startDate = formatDate(item.start_date);  // Định dạng lại ngày bắt đầu của voucher
    
            if (date > startDate) {
                // Kiểm tra ngày hiện tại có lớn hơn ngày bắt đầu voucher không
                console.log("Không hợp lệ", date, startDate);
            } else {
                console.log("Hợp lệ", date, startDate);
            }
        });
    }, [voucher]);
