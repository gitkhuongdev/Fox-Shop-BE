...
function Index({ data }) {
    ...
    const applyVoucher = (voucher) => {
        if (totalAmount >= voucher.minimum_monney) {
            ...
            setIdVch(voucher.id);
            setTimeout(() => {
                notyf.success(`Đã áp dụng voucher "${voucher.code}" thành công!`);
            }, 17000);
        } else {
            notyf.error(`Không đủ điều kiện áp dụng voucher. Tổng tiền tối thiểu: ${voucher.minimum_monney} VNĐ.`)
        }
    };
    useEffect(() => {
        try {
        ...
        try {
            if (paymentMethod === "cash") {
                const response = await axios.post("/api/orders", {
                    ...
                    id_payment: 1,
                    ...(idVch && idVch >= 0 ? { id_voucher: idVch } : {}),
                });
                if (showBtn) {
                    try {
                        const voucherResponse = await axios.delete(
                            "/api/voucher/user_vouchers",
                            {
                                data: {
                                    id_voucher: showBtn.id,
                                    id_user: userId,
                                },
                            }
                        );
